...
├── RMM-Deploy.ps1         # Generic RMM deploy wrapper — caches Run-Audit.ps1, monthly guard, token expansion
├── README.md              # User-facing documentation
└── LICENSE                # MIT 2025
```

**`Run-Audit.ps1` is the primary file to edit.** The `.exe` is regenerated by CI. `RMM-Deploy.ps1` is uploaded once to the RMM platform and only updated when its own deploy logic changes — it does not need recompiling. It carries its own `$DeployScriptVersion` variable (currently `"1.0.0"`) which must be bumped whenever the deploy script itself is changed. It accepts a `-RmmPlatform` parameter (default: `Atera`) to control platform-specific logging and output.
...
### 4. Hudu Integration Functions

All Hudu functions live in the `# Hudu API Functions` region, defined before the main execution sequence. Script-scope variables `$script:_HuduBaseURL`, `$script:_HuduAPIKey`, and `$script:_HuduCompanyId` are set during Hudu validation and shared by all helpers.

| Function | Purpose |
|---|---|
| `Invoke-HuduRequest` | Lightweight REST wrapper; adds `x-api-key` header, serialises body to JSON, supports GET/POST/PUT |
| `Get-HuduAssetLayoutByName` | Paginates all asset layouts; returns the first with a matching name |
...
Do **not** use hyphenated suffixes like `v1.3.2-1`. The self-update mechanism and `RMM-Deploy.ps1` both use `[System.Version]` for version comparison, which only accepts up to four numeric components (`major.minor.build.revision`). A hyphen causes a parse failure and falls back to unreliable string comparison.
