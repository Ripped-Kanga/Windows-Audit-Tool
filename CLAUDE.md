...
├── RMM-Atera-Deploy.ps1   # Atera deploy wrapper — caches Run-Audit.ps1, monthly guard, HuduEntryName token expansion
├── README.md              # User-facing documentation
└── LICENSE                # MIT 2025
```

**`Run-Audit.ps1` is the primary file to edit.** The `.exe` is regenerated by CI. `RMM-Atera-Deploy.ps1` is uploaded once to Atera and only updated when its own deploy logic changes — it does not need recompiling. It carries its own `$DeployScriptVersion` variable (currently `"1.2.0"`) which must be bumped whenever the deploy script itself is changed.
